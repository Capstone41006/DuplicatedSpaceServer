...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, args):
    ...
    train_cams_init = scene.getTrainCameras().copy()                                        #### sumin1 #####
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            if (iteration in saving_iterations):
                ...
                scene.save(iteration)                                                               #### sumin2 ####
                save_pose(scene.model_path + 'pose' + f"/pose_{iteration}.npy", gaussians.P, train_cams_init)                   #### 중요 - Dust3r 카메라 위치 저장 ####
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs):
    ...
    if iteration in testing_iterations:
        ...
        validation_configs = ({'name': 'test', 'cameras' : scene.getTestCameras()}, 
                              {'name': 'train', 'cameras' : [scene.getTrainCameras()[idx % len(scene.getTrainCameras())] for idx in range(len(scene.getTrainCameras()))]})          #### sumin3 ####
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    image = torch.clamp(renderFunc(viewpoint, scene.gaussians, *renderArgs, camera_pose=pose)["render"], 0.0, 1.0)                      # 예측 이미지: 뷰 포인트에서의 가우시안으로 이미지를 0~1로 저장
                    gt_image = torch.clamp(viewpoint.original_image.to("cuda"), 0.0, 1.0)                                                               # 정답 이미지: 원래 이미지에서의 값을 0~1로 저장
                    if tb_writer and (idx < 5):
                        tb_writer.add_images(config['name'] + "_view_{}/render".format(viewpoint.image_name), image[None], global_step=iteration)
                        if iteration == testing_iterations[0]:
                            tb_writer.add_images(config['name'] + "_view_{}/ground_truth".format(viewpoint.image_name), gt_image[None], global_step=iteration)
                    l1_test += l1_loss(image, gt_image).mean().double()                                                                                 # 평가 - l1 값 구함
                    psnr_test += psnr(image, gt_image).mean().double()                                                                                  # 평가 - psnr 값 구함
